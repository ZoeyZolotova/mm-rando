 ...
 if (spoiler.GossipHints != null && spoiler.GossipHints.Any()) { 
    ...
    foreach (var hint in spoiler.GossipHints.OrderBy(h => h.Key.ToString())) { 
            ...
            this.Write("<script>\r\n    function all(list, predicate) {\r\n        for (var i = 0; i < list.l" +
                    ...
                    "(event) {\r\n        if (event.keyCode === 13) {\r\n            loadItems();\r\n      " +
                    "  }\r\n    });\r\n\r\n    window.onbeforeunload = function() { return \"Are you sure yo" +
                    "u want to leave the page?\" }\r\n\r\n    function checkLocations(locations) {\r\n      " +
                    "  var itemsToCheck = [];\r\n        for (var i = 0; i < locations.length; i++) {\r\n" +
                    "            var location = logic[locations[i]];\r\n            location.IsAvailabl" +
                    "e = \r\n                (location.RequiredItemIds === null || location.RequiredIte" +
                    "mIds.length === 0 || all(location.RequiredItemIds, function(id) { return logic[i" +
                    "d].Acquired || logic[id].IsItemRemoved; }))\r\n                && \r\n              " +
                    "  (location.ConditionalItemIds === null || location.ConditionalItemIds.length ==" +
                    "= 0 || any(location.ConditionalItemIds, function(conditionals) { return all(cond" +
                    "itionals, function(id) { return logic[id].Acquired || logic[id].IsItemRemoved; }" +
                    "); }));\r\n            \r\n            var newLocation = find(logic, function(io) { " +
                    "return io.NewLocationId === locations[i]; });\r\n            if (!newLocation) {\r\n" +
                    "                newLocation = location;\r\n            }\r\n            if (!newLoca" +
                    "tion.Acquired && location.IsFakeItem && location.IsAvailable) {\r\n               " +
                    " newLocation.Acquired = true;\r\n                itemsToCheck.push(newLocation.Ite" +
                    "mId);\r\n            }\r\n            if (newLocation.Acquired && location.IsFakeIte" +
                    "m && !location.IsAvailable) {\r\n                newLocation.Acquired = false;\r\n  " +
                    "              itemsToCheck.push(newLocation.ItemId);\r\n            }\r\n        \r\n " +
                    "           var locationRows = document.querySelectorAll(\".item-replacements tr[d" +
                    "ata-newlocationid=\'\" + locations[i] + \"\']\");\r\n            for (const locationRow" +
                    ...
                    "eckbox.checked = location.Checked;\r\n                if (location.Checked) {\r\n   " +
                    "                 itemName.classList.remove(\"spoiler\");\r\n                } else {" +
                    "\r\n                    itemName.classList.add(\"spoiler\");\r\n                }\r\n   " +
                    "         }\r\n        \r\n            var itemRows = document.querySelectorAll(\"#ite" +
                    "m-locations tr[data-newlocationid=\'\" + locations[i] + \"\']\");\r\n            for (c" +
                    "onst itemRow of itemRows) {\r\n                var itemNames = itemRow.querySelect" +
                    "orAll(\".newlocation\");\r\n                var checkbox = itemRow.querySelector(\"in" +
                    "put\");\r\n                var item = logic[itemRow.dataset.id];\r\n                c" +
                    "heckbox.checked = item.Acquired;\r\n                for (const itemName of itemNam" +
                    "es) {\r\n                    if (item.Acquired) {\r\n                        itemNam" +
