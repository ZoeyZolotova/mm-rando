...
namespace MMR.Randomizer.Utils
{
    public class SequenceUtils
    {
        ...
        [
            AudioSequenceIds.MAJORAS_THEME,        // 0x04: Majora's Theme
            AudioSequenceIds.CLOCK_TOWER_INTERIOR, // 0x05: Clock Tower Interior
            AudioSequenceIds.BOAT_CRUISE,          // 0x0E: Old Koume's Boat Cruise
            AudioSequenceIds.SHARPS_CURSE,         // 0x0F: Sharp's Curse
            AudioSequenceIds.MUSIC_BOX_HOUSE,      // 0x27: Music-Box House
            AudioSequenceIds.ZELDAS_THEME,         // 0x29: Zelda's Theme
            AudioSequenceIds.GIANTS_THEME,         // 0x2D: Giants' Theme
            AudioSequenceIds.GURU_GURUS_THEME,     // 0x2E: Guru-Guru's Theme
            AudioSequenceIds.MAYORS_OFFICE,        // 0x31: Mayor Dotour's Office
            AudioSequenceIds.GORMAN_BROS_THEME,    // 0x42: Gorman Bros.' Theme
            AudioSequenceIds.OWLS_THEME,           // 0x45: Kaepora Gaebora's Theme
            AudioSequenceIds.SWORDSMANS_SCHOOL,    // 0x50: Swordsman's School
            AudioSequenceIds.GIANTS_APPEAR,        // 0x70: The Giants Appear
            AudioSequenceIds.CREMIAS_THEME,        // 0x72: Cremia's Theme
            AudioSequenceIds.KEATONS_THEME,        // 0x73: Keaton's Theme
            AudioSequenceIds.MOON_ENRAGED,         // 0x7B: The Moon Enraged
            AudioSequenceIds.GIANTS_LEAVE,         // 0x7C: The Giants Leave
            AudioSequenceIds.REUNION_THEME,        // 0x7D: Reunion Theme
        ];
        ...
        /// </summary>
        public static InstrumentBankUtils.Audiobin MM_AUDIOBIN = null;
        ...
        /// </summary>
        public static InstrumentBankUtils.Audiobin OOT_AUDIOBIN = null;
        ...
        public static void LoadMMAudiobin()
        {
            ...
            MM_AUDIOBIN = new InstrumentBankUtils.Audiobin(mmAudiobankFile, mmAudiobankIndex, mmAudiotableFile, mmAudiotableIndex);
        }
        ...
        public static void LoadOOTAudiobin()
        {
            ...
            try
            {
                ...
                OOT_AUDIOBIN = new InstrumentBankUtils.Audiobin(ootAudiobank, ootAudiobankIndex, ootAudiotable, ootAudiotableIndex);
            }
        ...
        public static void ReadSequenceInfo()
        {
            ...
            foreach (var entry in sequenceEntries)
            {
                ...
                if (AudioSequenceIds.SEQUENCE_ID_MAP.ContainsKey(seqId) && seqId >= 0x02 && seqId <= 0x7F)
                {
                ...
                if (sourceSequence.SeqId != AudioSequenceIds.FILE_SELECT && sourceSequence.Name != "drop")
                {
            ...
            RomData.SequenceList.Add(new SequenceInfo
            {
                ...
                Instrument = 0x03,
                Replaces = AudioSequenceIds.INTRO_CUTSCENE_2,
            });
            ...
            // DEBUG: Music cache causes headaches during debug...
# if RELEASE
        ...
        public static void ProcessCustomMusicFile(string filePath)
        {
            try
            {
                using (ZipArchive zip = ZipFile.OpenRead(filePath))
                {
                    ...
                    if (metadata.Game == "oot" && !MusicHelpers.IsMMRSFile(currentSong.Filepath) ||
                        metadata.Game != "oot" && MusicHelpers.IsOOTRSFile(currentSong.Filepath))
                        LoadOOTAudiobin();
        ...
        private static MusicMetadata ReadMusicMetadataYaml(string songname, ZipArchiveEntry metaFile)
        {
            ...
            {
                throw new Exception($"Error for: {songname}", e);
            }
        ...
        private static SequencePlayState[] ConvertFormmaskDict(Dictionary<string, List<string>> formmask)
        {
            ...
            static SequencePlayState ParseStatesAndForms(List<string> states)
            {
                // Parses the states and forms from a formmask dictionary list and returns the correct SequencePlayState
                SequencePlayState combined = SequencePlayState.None;
                if (states != null)
                {
                    foreach (var s in states)
                    {
                        if (Enum.TryParse<SequencePlayState>(s, true, out var state))
                        {
                            combined |= state;
                        }
                    }
            ...
            for (int i = 0; i < MaxChannels; i ++)
            {
                ...
                if (formmask.TryGetValue(key, out var states))
                {
                    result[i] = ParseStatesAndForms(states);
                } 
            }

            // Get the cumulative states
            if (formmask.TryGetValue("cumulative states", out var cumulativeStates))
                result[CumulativeIndex] = ParseStatesAndForms(cumulativeStates);
        ...
        private static bool ReadMusicInstrumentBank(SequenceInfo song, SequenceBinaryData combo, ZipArchiveEntry bankFile, ZipArchiveEntry bankmetaFile)
        {
            ...
            if (bankFile != null && bankmetaFile != null)
            {
                ...
                if (song.Game == "oot" && !MusicHelpers.IsMMRSFile(song.Filepath) ||
                    song.Game != "oot" && MusicHelpers.IsOOTRSFile(song.Filepath))
            ...
            else if (song.Game == "oot" && !MusicHelpers.IsMMRSFile(song.Filepath) && song.Instrument < 0x25 ||
                     song.Game != "oot" && MusicHelpers.IsOOTRSFile(song.Filepath) && song.Instrument < 0x25) // Vanilla OOTR files require a custom bank
            {
                ...
                var vanillaOOTBank = new InstrumentBankUtils.InstrumentBank(bankmeta, OOT_AUDIOBIN.Audiobank, OOT_AUDIOBIN.Audiotable, OOT_AUDIOBIN.AudiotableIndex);

                // The audiotable should be 1, and the audiobin should correct all sample data to use AT1
                // The bankmeta stored in the audiobank class is 8 bytes, not 16
                vanillaOOTBank.BankMetadata[0x02] = 1;

                // Modify OOT samples, checking if MM contains their data to update their sample addresses
                // or if the sample data needs to be used as a custom audio sample
                vanillaOOTBank.BankData = ProcessOOTSampleData(song, vanillaOOTBank.BankData, vanillaOOTBank.BankMetadata);

                combo.InstrumentSet = new InstrumentSetInfo()
                {
                    BankBinary = vanillaOOTBank.BankData,
                    BankSlot = REQUIRES_NEW_BANK,
                    BankMetaData = vanillaOOTBank.BankMetadata,
                    Modified = 1,
        ...
        {
            var newBank = new InstrumentBankUtils.InstrumentBank(bankmetaData, bankData, OOT_AUDIOBIN.Audiotable, OOT_AUDIOBIN.AudiotableIndex);
        ...
        public static int GetSequenceSize(SequenceInfo seq)
        {
            ...
            {
                return AlignmentHelpers.AlignTo16(seq.SequenceBinary.SequenceData.Length);
            }
            else if (AudioSequenceIds.SEQUENCE_ID_MAP.ContainsKey(seq.SeqId)) // If seq is vanilla, then SeqId is set and Replaces is -1; lookup from AudioSeq index table
            {
                // The code file ahould already be decompressed
                int codeFID = RomUtils.GetFileIndexForWriting(Addresses.AUDIOSEQ_TABLE);
                var codeFile = RomData.MMFileList[codeFID];
                int audioseqIndexTableOffset = Addresses.AUDIOSEQ_TABLE - codeFile.Addr;
                ...
                return AlignmentHelpers.AlignTo16(size);
            }
            else // The sequence is not loaded into memory, so search for the sequence file
            {
                if (File.Exists(seq.Filename))
                {
                    ...
                    return AlignmentHelpers.AlignTo16((int)length);
                }
            }
        ...
        public static void PointerizeSequenceSlots()
        {
            ...
            //
            ConvertSequenceSlotToPointer(AudioSequenceIds.ZELDAS_THEME, AudioSequenceIds.SONG_OF_HEALING_THEME);
            ...
            if (shortenedCutscenes)
            {
                ...
                ConvertSequenceSlotToPointer(AudioSequenceIds.CREMIAS_THEME, AudioSequenceIds.OWLS_THEME);    
                ConvertSequenceSlotToPointer(AudioSequenceIds.GIANTS_THEME, AudioSequenceIds.ASTRAL_OBSERVATORY);
                ConvertSequenceSlotToPointer(AudioSequenceIds.GIANTS_APPEAR, AudioSequenceIds.SONG_OF_HEALING_THEME);
                ConvertSequenceSlotToPointer(AudioSequenceIds.MOON_ENRAGED, AudioSequenceIds.ALIENS_THEME);
                ConvertSequenceSlotToPointer(AudioSequenceIds.REUNION_THEME, AudioSequenceIds.CLOCK_TOWER_INTERIOR);
            }
            ...
            {
                ConvertSequenceSlotToPointer(AudioSequenceIds.MAJORAS_THEME, AudioSequenceIds.SMALL_ENEMY_BATTLE);
            }
            ...
            {
                ConvertSequenceSlotToPointer(AudioSequenceIds.TITLE_DEMO, AudioSequenceIds.CLOCK_TOWN_1);
                ConvertSequenceSlotToPointer(AudioSequenceIds.EVENT_FAIL_1, AudioSequenceIds.EVENT_FAIL_2);
                ConvertSequenceSlotToPointer(AudioSequenceIds.EVENT_SUCCESS, AudioSequenceIds.TEMPLE_CLEAR_SHORT);
            }

            // create some pointerized slots that are otherwise ignored, beacuse this pool gets re-used later for new song slots
            RomData.PointerizedSequences.Add(new SequenceInfo() { Name = "mm-introcutscene1", SeqId = AudioSequenceIds.INTRO_CUTSCENE_1, PreviousSlot = AudioSequenceIds.INTRO_CUTSCENE_1, Replaces = AudioSequenceIds.TITLE_DEMO });
        }
        ...
        public static void RebuildAudioSeq(List<SequenceInfo> sequenceList, int? sequenceMaskFileIndex, int? sequenceNamesFileIndex)
        {
            ...
            int f = RomUtils.GetFileIndexForWriting(Addresses.AUDIOSEQ_TABLE);
            ...
            for (int i = 0; i < 128; i++)
            {
                ...
                int entryaddr = Addresses.AUDIOSEQ_TABLE + (i * 16);
                ...
                else
                {
                    ...
                    if (j != -1)
                    {
                        ...
                        {
                            if (sequenceList[j].Replaces != AudioSequenceIds.POINTER_0x18) // 0x28: Great Fairy's Fountain
                            {
                                sequenceList[j].Replaces = entry.Addr;
                            }
                            else
                            {
                                entry.Data = oldSeq[AudioSequenceIds.FILE_SELECT].Data;
                            }
                        }
                    }
                }
            ...
            f = RomUtils.GetFileIndexForWriting(Addresses.AUDIOSEQ_TABLE);
            for (int i = 0; i < 128; i++)
            {
                ReadWriteUtils.Arr_WriteU32(RomData.MMFileList[f].Data, (Addresses.AUDIOSEQ_TABLE + (i * 16)) - basea, (uint)newSeq[i].Addr);
                ReadWriteUtils.Arr_WriteU32(RomData.MMFileList[f].Data, 4 + (Addresses.AUDIOSEQ_TABLE + (i * 16)) - basea, (uint)newSeq[i].Size);
            }
            ...
            f = RomUtils.GetFileIndexForWriting(Addresses.INST_SET_MAP);
            basea = RomData.MMFileList[f].Addr;
            for (int i = 0; i < 128; i++)
            {
                // huh? paddr? pointer? padding?
                int paddr = (Addresses.INST_SET_MAP - basea) + (i * 2) + 2;
        ...
        public static void CheckSongTest(List<SequenceInfo> sequences, StringBuilder log)
        {
            ...
            SequenceInfo fileselectSlot = RomData.TargetSequences.Find(u => u.Replaces == AudioSequenceIds.FILE_SELECT); // Don't rely on the name in the SEQS.txt, rely on the sequence ID instead
            ...
            ConvertSequenceSlotToPointer(AudioSequenceIds.TITLE_DEMO, AudioSequenceIds.FILE_SELECT);
            ConvertSequenceSlotToPointer(AudioSequenceIds.CLOCK_TOWN_1, AudioSequenceIds.FILE_SELECT);
            ...
            foreach (SequenceInfo songslot in allMatchingSlots)
            {
                ...
                if (songslot.Replaces == AudioSequenceIds.SMALL_ENEMY_BATTLE)
                    continue;

                ConvertSequenceSlotToPointer(songslot.Replaces, AudioSequenceIds.FILE_SELECT);
            }
        ...
        public static bool SearchForValidSongReplacement(CosmeticSettings cosmeticSettings, List<SequenceInfo> unassignedSequences, SequenceInfo targetSlot, Random rng, StringBuilder log)
        {
            ...
            foreach (var testSeq in unassignedSequences.ToList())
            {
                ...
                var maxSize = targetSlot.Replaces == AudioSequenceIds.SMALL_ENEMY_BATTLE ? MAX_COMBAT_BUDGET : MAX_BGM_BUDGET;
        ...
        public static void CheckBGMCombatMusicBudget(CosmeticSettings cosmeticSettings, List<SequenceInfo> unassignedSequences, Random rng, StringBuilder log)
        {
            ...
            var BGMSlots = RomData.TargetSequences.FindAll(u => u.Categories.Contains((int)MusicGroups.Category.Fields)
                                                             ...
                                                             || u.SeqId == AudioSequenceIds.DEKU_PALACE); // 0x12: Deku Palace (has enemies)
            ...
            var usedCombatSequence = RomData.SequenceList.Find(u => u.Replaces == AudioSequenceIds.SMALL_ENEMY_BATTLE && u.SeqId != AudioSequenceIds.SMALL_ENEMY_BATTLE); // SequencesList has Replaces as -1, use SeqId (u.Name != "mm-combat")
            if (usedCombatSequence == null) // Songtest removes the sequence and points it at "File Select" for testing
            {
                ...
                usedCombatSequence = RomData.SequenceList.Find(u => u.Replaces == AudioSequenceIds.FILE_SELECT && u.SeqId != AudioSequenceIds.FILE_SELECT); // SequencesList has Replaces as -1, use SeqId (u.Name != "mm-fileselect")
            }
            ...
            else // BGM chosen
            {
                ...
                if (combatSize > newCombatBudget)
                {
                    ...
                    var combatSlot = RomData.TargetSequences.Find(u => u.Replaces == AudioSequenceIds.SMALL_ENEMY_BATTLE); // TargetSequences has SeqId as -1, use Replaces (u.Name == "mm-combat")
        ...
        public static void ReadInstrumentSetList()
        {
            ...
            for (int audiobankIndex = 0; audiobankIndex <= 0x80; ++audiobankIndex)
            {
                ...
                {
                    bankData[b] = ReadWriteUtils.Read(Addresses.AUDIOBANK_ADDR + audiobankBankOffset + b);
                }
            ...
            foreach (var instrumentset in RomData.InstrumentSetList)
            {
                if (instrumentset.InstrumentSamples != null && instrumentset.InstrumentSamples.Count > 0)
                {
                    foreach (var sample in instrumentset.InstrumentSamples)
                    {
                        ...
                        InstrumentBankUtils.InstrumentBank instrumentBank = null;
                        try
                        {
        ...
        public static void RebuildAudioBank(List<InstrumentSetInfo> InstrumentSetList)
        {
            ...
            var audiobankFile = RomData.MMFileList[RomUtils.GetFileIndexForWriting(Addresses.AUDIOBANK_ADDR)];
